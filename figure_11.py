...
# Title
title = "Figure 11: Session Duration (IQR-filtered)"
...
df['vsession_duration(hours)'] = df['vsession_duration(min)'] / 60

# Filter the column (now using hours)
inter_api_duration_filtered = filter_iqr(df['vsession_duration(hours)'])
...
plt.xlabel('Session Duration (hours, IQR-filtered)', fontsize=14, labelpad=15)
